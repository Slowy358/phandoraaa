...
client.on("guildMemberAdd", member => {  
  const kanal = "790854423895080970";
...
client.on("guildMemberAdd", member => {
  ...
  moment.locale("tr")
   ...
   var rol = member.guild.roles.get("790596063149031474") ///Cezalı Rol ID'si
   var kayıtsız = member.guild.roles.get("790514380063244298") ///Kayıtsız rolü ID'si
   member.addRole(rol)
...
client.on("userUpdate", async (eski, yeni) => {
  ...
  var ekipRolü = "790594677874753556"; // Tagın Rol IDsi
  var logKanali = "791547439160885278"; // Loglanacağı Kanalın ID
  ...
  if ((yeni.username).includes(ekipTag) && !uye.roles.has(ekipRolü)) {
    try {
      ...
      await client.channels.get(logKanali).send(`${yeni} adlı üye tagımızı alarak <@&790594677874753556> rolünü kazandı`);
  ...
  if (!(yeni.username).includes(ekipTag) && uye.roles.has(ekipRolü)) {
    try {
      ...
      await client.channels.get(logKanali).send(`${yeni} adlı üye tagımızı bırakarak <@&790594677874753556> rolünü kaybetti  `);
    } catch(err) { console.error(err) };
  };
});
...
client.on("ready", () => {
  client.channels.get("791218362482819092").join();
